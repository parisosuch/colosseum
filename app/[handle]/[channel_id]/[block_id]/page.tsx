...
import { Column, getColumn, getBlockChannels } from "@/lib/colosseum/column";
...
export default async function BlockPage({ params }: BlockPageParams) {
  ...
  // a block that isn't connected to the channel the URL claims) is treated as
  // not found rather than leaking its existence. A block can live in multiple
  // channels, so membership is checked against block_channel.
  const column = await getColumn(supabase, parseInt(block_id, 10));
  const blockChannels = column ? await getBlockChannels(supabase, column.id) : [];
  const notFound = !column || !blockChannels.some((c) => c.id === channelId);
